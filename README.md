...
In order to mimic the auto-solver, use the pop-up menus in the upper left corner of the puzzle window.  Then select the "Process Edges" menu item.  Internally, this calls the ProcessEdges function repeatedly until nothing further changes.  Thereafter, select "Process inner rows" followed by "Process inner columns".  Repeat these last two steps until no further changes are made.  At which point, you'll need to choose a new guess and start over with "Process Edges".
